...
or (p.status = 'public' and sgu.status = 'private' and sgm.user_id = 'user124' and sgm.status != 'delete') 
or (p.status = 'friend' and exists(select 1 from friendships where ((user_id_1 = p.user_id and user_id_2 = 'user124') or (user_id_1 ='user124'  and user_id_2 = p.user_id)) and status != 'delete') and sgu.status = 'public'  ) 
or (p.status = 'friend' and sgu.status = 'private' and sgm.user_id = 'user124' and sgm.status != 'delete' and exists(select 1 from friendships where (user_id_1 = p.user_id and user_id_2 = 'user124') 
or (user_id_1 ='user124'  and user_id_2 = p.user_id))) 
or (p.status = 'private' and sgu.status = 'public' and p.user_id = 'user124') 
or (p.status = 'private' and sgu.status = 'private' and p.user_id = 'user124' and sgm.user_id = 'user124')))
or (p.share_id is null and ((p.status = 'public') 
or (p.status = 'friend' and exists(select 1 from friendships where ((user_id_1 = p.user_id and user_id_2 = 'user124') or (user_id_1 ='user124'  and user_id_2 = p.user_id)) and status != 'delete') )  
or (p.status = 'private' and p.user_id = 'user124')
or (p.status = 'privateGroup' and gu.status = 'private' and gm.user_id = 'user124' and gm.status != 'delete')
))
group by p.post_id
) as pn
...
group by pn.post_id
order by field(exists(select 1 from friendships where ((user_id_1 = pn.user_id and user_id_2 = 'user124') or (user_id_1 ='user124'  and user_id_2 = pn.user_id)) and status != 'delete'),True,False),pn.created_at desc,field(pn.share_id is null,True,False),FIELD(pn.type_post,'user','group','page')
`;
